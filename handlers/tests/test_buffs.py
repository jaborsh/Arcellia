...
from handlers.buffs import BaseBuff, BuffableProperty, BuffHandler, Mod
...
class TestBuffsAndHandler(EvenniaTest):
    ...
    def test_getters(self):
        ...
        # stat getters
        self.assertTrue(
            isinstance(handler.get_by_stat("stat1")["tmb"], _TestModBuff)
        )
    ...
    def test_context_conditional(self):
        ...
        self.testobj.db.cond1, self.testobj.db.att, self.testobj.db.dmg = (
            False,
            None,
            0,
        )
        ...
        self.assertEqual(
            handler.get_by_type(_TestConBuff)["tcb"].conditional(
                **_testcontext
            ),
            False,
        )
    ...
    def test_complex(self):
        ...
        handler: BuffHandler = self.testobj.buffs
        (
            self.testobj.db.comone,
            self.testobj.db.comtwo,
            self.testobj.db.comtext,
        ) = 10, 0, {}
        ...
        self.assertEqual(
            handler.get_by_type(_TestComplexBuff)["tcomb"].conditional(
                **self.testobj.db.comtext
            ),
            False,
        )
        self.assertEqual(
            handler.check(
                self.testobj.db.comone, "com1", context=self.testobj.db.comtext
            ),
            10,
        )
        self.assertEqual(
            handler.check(
                self.testobj.db.comtwo, "com2", context=self.testobj.db.comtext
            ),
            0,
        )
