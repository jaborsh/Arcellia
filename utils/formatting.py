...
def wrap(text, text_width=80, pre_text="", align="l"):
    ...
    for text_line in text_lines:
        ...
        for word in word_list:
            ...
            # Catch words that are too long to fit on a line, for whatever reason.
                ...
                char_list = re.findall(
                    ...
                    r"\|\[?([0-5][0-5][0-5]|\=?[rRyYgGcCbBmMwWxX]|#?[0-9a-f]{6})", word
                )
        ...
        for line in line_list:
            ...
            if first_line:
                line_text = justify(line, text_width, align, 0)
                first_line = False
            else:
...
def justify(text, width, align, indent):
    if align == "l":
        return " "*indent + text
    elif align == "c":
        return " "*math.ceil(
            (width/2) - (len(strip_ansi(text))/2) + indent
        ) + text
