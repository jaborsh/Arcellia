...
def _option_list(*suboptions):
    ...
    if not xymap_data:
        if xyzgrid.db.map_data:
            ...
            print(
                "The XYZgrid is currently empty. Use 'add' to add paths to your map data."
            )
        return
    ...
    if not xymap:
        ...
        print(
            "Legend (all elements may not be present on map):\n " + "\n ".join(legend)
        )
...
def _option_initpath(*suboptions):
    ...
    for inum, xymap in enumerate(xymaps):
        print(
            f"(Re)building pathfinding matrix for xymap Z={xymap.Z} ({inum+1}/{nmaps}) ..."
        )
...
def xyzcommand(*args):
    ...
    if option in ("help", "h"):
        ...
        print(
            f"Unknown option '{option}'. Use 'evennia xyzgrid help' for valid arguments."
        )
