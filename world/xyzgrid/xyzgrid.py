...
class XYGrid(DefaultScript):
    ...
    def delete(self):
        ...
        if mapdata:
            self.remove_map(
                *(zcoord for zcoord in self.db.map_data), remove_objects=True
            )
        super().delete()
    ...
    def spawn(self, xyz=("*", "*", "*"), directions=None):
        ...
        if z == wildcard:
            ...
            raise RuntimeError(
                f"The 'z' coordinate/name '{z}' is not found on the grid."
            )
...
def get_xyzgrid(print_errors=True):
    ...
    """
    xyzgrid = XYGrid.objects.all()
    if not xyzgrid:
        # create a new one
        xyzgrid, err = XYGrid.create("XYZGrid")
        if err:
