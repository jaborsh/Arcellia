...
class XYZMobManager(XYZManager):
    ...
    Manager that also allows searching for mobs based on XY coordinates.
    """

    def filter_xyz_mob(self, xyz=("*", "*", "*"), **kwargs):
        """
        Used by mobs (objects with a source property).
        Find all mobs in a source location. This will also find
        children of XYZMob on the given coords..

        Kwargs:
            xyz (tuple, optional): A coordinate (X, Y, Z) for the source location. Each
                element is either an `int` or `str`. The character `'*'` is used as a wildcard -
                so setting all coordinates to the wildcard will return *all* XYZMobs.
                the `Z`-coordinate is the name of the map (case-sensitive) in the XYZgrid contrib.
            **kwargs: All other kwargs are passed on to the query.
        ...
        Notes:
            Depending on what coordinates are set to `*`, this can be used to
            e.g. find all mobs in a room, or leading to a room or even to rooms
            in a particular X/Y row/column.
    ...
    def get_xyz_mob(self, xyz=(0, 0, "map"), **kwargs):
        """
        Used by mobs (objects with a source property). Get a single
        mob. All source coordinates (as well as the map's name) are required.
        This will also find children of XYZMob on the given coords.
        ...
        Returns:
            XYZMob: A single mob instance found at the combination of x, y and xgiven.

        Raises:
            XYZMob.DoesNotExist: If no matching query was found.
            XYZMob.MultipleObjectsReturned: If more than one match was found (which should not
                be possible with a unique combination of x,y,x).
