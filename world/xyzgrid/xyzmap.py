...
class XYZMap:
    ...
    def parse(self):
        ...
        if -1 in (topleft_marker_x, topleft_marker_y):
            raise MapParserError(
                f"No top-left corner-marker ({mapcorner_symbol}) found!"
            )
    ...
    def calculate_path_matrix(self, force=False):
        ...
        if (
            not force
            and self.pathfinder_baked_filename
            and isfile(self.pathfinder_baked_filename)
        ):
        ...
        if self.pathfinder_baked_filename:
            ...
            with open(self.pathfinder_baked_filename, "wb") as fil:
                pickle.dump(
                    (self.mapstring, self.dist_matrix, self.pathfinding_routes),
                    fil,
                    protocol=4,
                )
    ...
    def spawn_nodes(self, xy=("*", "*")):
        ...
        if not _XYZROOMCLASS:
            from world.xyzgrid.xyzroom import XYZRoom as _XYZROOMCLASS
        x, y = xy
        wildcard = "*"
        spawned = []
    ...
    def get_visual_range(
        ...
        if dist is None:
            ...
            points, xmin, xmax, ymin, ymax = self._get_topology_around_coord(
                xy, dist=dist
            )
