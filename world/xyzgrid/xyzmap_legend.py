        ...
        for direction, (dx, dy) in MAPSCAN.items():
            ...
            if lx in xygrid and ly in xygrid[lx]:
                ...
                if end_node:
                    ...
                    first_step_name = steps[0].direction_aliases.get(
                        direction, direction
                    )
        ...
        if not self.directions:
            ...
            if n_unhandled:
                ...
                if n_unhandled != 2:
                    ...
                    raise MapParserError(
                        f"cannot determine how to connect in/out directions {links}.",
                        self,
                    )
...
class SmartTeleporterMapLink(MapLink):
    ...
    def get_direction(self, start_direction):
        ...
        if not self.directions:
            ...
            if len(neighbors) != 1:
                raise MapParserError(
                    "must have exactly one link connected to it.", self
                )
            ...
            if start_direction == direction_name:
                ...
                self.directions = {
                    start_direction: direction_name,
